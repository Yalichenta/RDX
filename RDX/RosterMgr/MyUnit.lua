...
function RRR()
	...
	local mapFileName = C_Map.GetMapInfo(C_Map.GetBestMapForUnit("player"))["name"]
...
local function ProcessMyUnit(self, elapsed)
	...
	isIndoor = false
	cmaId = C_Map.GetBestMapForUnit("player") --GetCurrentMapAreaID()
	gcmd = 1 --GetCurrentMapDungeonLevel();
	mapId = nil;
	...
	plZX, plZY = 0.5, 0.5 --C_Map.GetPlayerMapPosition(C_Map.GetBestMapForUnit("player"),"player");
	plZX = plZX * 100
	plZY = plZY * 100
