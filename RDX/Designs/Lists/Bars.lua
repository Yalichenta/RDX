...
RDX.RegisterFeature({
	...
	local ]] .. objname .. [[_fnames = ]];
				if desc.externalNameFiltercd then
					closureCode = closureCode .. [[RDXDB.GetObjectInstance(]] .. string.format("%q", desc.externalNameFiltercd) .. [[);
	]];
				else
					-- Internal filter
					closureCode = closureCode .. [[{};
	...
	while true do
		...
		if (not _meta.isInvisible) and ]] .. aurasfilter .. [[ and ]] .. isstealablefilter .. [[ and ]] .. curefilter .. [[ and ]] .. timefilter .. [[ and ]] .. namefilter .. [[ then
			...
			if btn.nametxt then
				if ]] .. auranameab .. [[ then
					word, text = nil, "";
					for word in string.gmatch(_meta.text, "%a+")
						do text = text .. word:sub(1, 1);
					end
					btn.nametxt:SetText(text);
				elseif ]] .. auranametrunc .. [[ then
					btn.nametxt:SetText(strsub(_meta.text, 1, ]] .. auranametrunc .. [[));
				else
					btn.nametxt:SetText(_meta.text);
				end
			end
...
if band(paintmask, ]] .. mask .. [[) ~= 0 then
	...
	UIFromDescriptor = function(desc, parent, state)
		...
		ui:InsertFrame(VFLUI.Separator:new(ui, VFLI.i18n("Text parameters")));
		...
		chk_timefilter:SetText(VFLI.i18n("Show only bars with timer"));
		...
		ftype_1:SetText(VFLI.i18n("Use Aura Bars"));
		local ftype_2 = ftype:CreateRadioButton(ui);
		ftype_2:SetText(VFLI.i18n("Use Coodown Bars"));
		...
		ui:InsertFrame(VFLUI.Separator:new(ui, VFLI.i18n("Cooldown Bars")));
		...
		if desc and desc.filterName then chk_filterNamecd:SetChecked(true); else chk_filterNamecd:SetChecked(); end
