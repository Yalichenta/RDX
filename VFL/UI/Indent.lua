...
-- new as of lua 5.1
   ...
   -- fixme bethan 20/01/2019
   --  the original implementation is broken since 8.1 as editbox:Insert("\1") no longer
   -- seem to work. However GetCursorPosition seems to be implemented since 2.3 so let's
   -- use it instead
   ...
   if(-1 == pos) then
	return lib.stripWowColors(code), 0
   end
   ...
   function lib.padWithLinebreaks(code)
      ...
      if stringbyte(code, len) == linebreak then
        if stringbyte(code, len - 1) == linebreak then
          while(len > 3) do
            if stringbyte(code, len - 2) == linebreak then
              len = len -1
              code = code:sub(1,-2)
	      modified = true
            else
